...
class OmFtthValidation
{
    public static function Omvalidate($orgData,$problemSolved,$ActivityName ,$errorName,$ActivityCreateTime)
    {
        ...
        if($ActivityName == 'Resurvey' || $ActivityName == 'Resurvey Problem'){
            $ActivityName = 'Survey' ;
        }
        ...
        if($ActivityName == 'WO Request'){
            ...
            $withinSR = 'FBB Non Tech follow up-FTTH Order Installation-WO Request Within SLA';
            $delaySR = 'FBB Non Tech follow up-FTTH Order Installation-WO Request Within SLA';
            ...
            if(in_array($errorName, $CustomerProblem) ){
                ...
                $withinSR = 'FBB Non Tech follow up -> FTTH Order Installation-> Customer Problem informed';
                $delaySR = $withinSR;
                $ActivitySLA = '';

            }elseif(in_array($errorName, $LineProblem)){
                $actionMessage = ' ';
                $withinSR = 'FBB Non Tech follow up -> FTTH Order Installation-> WO-Line Problem Within SLA';
                $delaySR = 'FBB Non Tech follow up -> FTTH Order Installation -> WO-Line Problem After SLA';
                $ActivitySLA = "15WDs";
                ...
                if($problemSolved == 0){
                    $actionMessage = ' ';
                    $withinSR = 'FBB Non Tech follow up->FTTH Order Installation->WO-TE Problem Within SLA';
                    $delaySR = 'FBB Non Tech follow up-FTTH Order Installation-WO-TE Problem After SLA';
                    $ActivitySLA = "7WDs";
                    $orderSLA = $ActivitySLA;
                }else{
            ...
            $withinSR = 'FBB Non Tech follow up -> FTTH Order Installation -> Check Combo Within SLA';
            $delaySR = 'FBB Non Tech follow up -> FTTH Order Installation -> Check Combo After SLA';
            ...
            if($errorName == 'No Free Ports'){
                $actionMessage = '';
                $withinSR = 'FBB Non Tech follow up -> FTTH Order Installation -> Reserve activation no estimated time';
                $delaySR = $withinSR;
            ...
            $AutomaticTT = '';
        }elseif($errorName == 'Get FTTH Path API Failed'){
            $withinSR = '';
            $delaySR = '';
            $ActivitySLA = "";
            $AutomaticTT = '';

            $actionMessage = 'will be considered as no free port issue and we will inform CST with the below message: <br> فى حاله سؤال العميل عن سبب التأخير في تقديم الخدمة يجب إخبار العميل أنه يتم تحديث المعدات الخاصة بـينا في السنترال نظرا لإزدياد العملاء ولذلك يجب تحديث المعدات كل فترة ولا يتم إخبار العميل أنه توجد مشكلة عامة في السنترال';
            $withinSR = 'FBB Non Tech follow up-> FTTH Order Installation -> Survey no estimated time';
            $delaySR = $withinSR;
            $ActivitySLA = "No Estimated time";
            $orderSLA = $ActivitySLA;
            ...
            $actionMessage = '';
            $withinSR = 'FBB Non Tech follow up -> FTTH Order Installation -> Splitting Within SLA normal cycle';
            $delaySR = 'FBB Non Tech follow up -> FTTH Order Installation -> Splitting After SLA';
            ...
            if(in_array($errorName, $CustomerProblem)){
                ...
                $actionMessage = '';
                $withinSR = 'FBB Non Tech follow up -> FTTH Order Installation -> Splitting-Line Problem Within SLA';
                $delaySR = 'FBB Non Tech follow up -> FTTH Order Installation -> Splitting-Line Problem After SLA';
                $ActivitySLA = "15 WD";
                $delayTT = '';
            }elseif(in_array($errorName, $TEProblem)){
            ...
            if($errorName == 'No Free Ports'){
                $withinSR = 'FBB Non Tech follow up -> FTTH Order Installation -> Port Problem';
                $delaySR = '';
                $delayTT = '';
                $ActivitySLA = "No Estimated time";
                $orderSLA = $ActivitySLA;
            }elseif($errorName == 'Outage (Global Problem)' || $errorName == 'Outage'||$errorName == 'Global Problem'){
                $withinSR = 'FBB Non-Tech Inquiry -> FTTH Order Installation -> Outage ( Global problem )';
                $delaySR = '';
                $delayTT = '';
                $ActivitySLA = "No Estimated time";
                $orderSLA = $ActivitySLA;
            ...
            if($errorName == 'Voice Provisioning Configuration'){
                ...
                $withinSR = 'FBB Non Tech follow up -> FTTH Order Installation -> Voice provisioning within SLA';
                $delaySR = 'FBB Non Tech follow up -> FTTH Order Installation -> Voice provisioning After SLA';

            }elseif($errorName == 'Network Activation Configuration'){
                $withinSR = 'FBB Non Tech follow up -> FTTH Order Installation -> Network Activation Within SLA';
                $delaySR = 'FBB Non Tech follow up -> FTTH Order Installation -> Network Activation After SLA';
            ...
            $withinSR = 'FBB Non Tech follow up -> FTTH Order Installation -> IM Update Within SLA';
            $delaySR = 'FBB Non Tech follow up -> FTTH Order Installation -> IM Update After SLA';
            ...
            if($errorName == 'Home Visit dispatch'){
                ...
                $withinSR = 'FBB Non Tech follow up -> FTTH Order Installation -> Home Visit Within SLA';
                $delaySR = 'FBB Non Tech follow up -> FTTH Order Installation -> Home Visit After SLA';
            ...
            $withinSR = 'FBB Non Tech follow up -> FTTH Order Installation -> Check SN Within SLA';
            $delaySR = 'FBB Non Tech follow up -> FTTH Order Installation -> Check SN After SLA';
        }elseif($ActivityName == 'Call Back TO CRM' || $ActivityName == 'Call back to CRM'){
            $withinSR = 'FBB Non Tech follow up -> FTTH Order Installation -> Call back to CRM';
            $AutomaticTT = '';
